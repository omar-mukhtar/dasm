...
CBPAView::CBPAView() : MAX_CHAR_PER_LINE(56), MAX_LINES(8)
{
    iDataArray = NULL;
}
...
void CBPAView::Draw(const TRect& /* aRect */) const
{
	...
	_LIT(K_BANNERR_STR, "Bundle Protocol App. : Omar Mukhtar, Joerg Ott (HUT)");
	_LIT(K_BANNERR_STR2, "--------------------------------------------------------------------------------");
...
void CBPAView::PrintMsg(const TDesC16& aMsg)
{
	AppendLine(aMsg);
	DrawNow();
}

void CBPAView::PrintMsg(const TDesC8& aMsg)
{
	HBufC16* tmpBuff = HBufC16::New(aMsg.Length());
	if(NULL != tmpBuff)
	{
		tmpBuff->Des().Copy(aMsg);
		PrintMsg(*tmpBuff);
		delete tmpBuff;
	}
}
