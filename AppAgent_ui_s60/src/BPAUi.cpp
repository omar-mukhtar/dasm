...
void CBPAUi::ConstructL()
{
	...
	iBPA = CBPA::NewL(K_DEFAULT_TIME_PERIOD);
}
...
void CBPAUi::HandleCommandL(TInt aCommand)
{
	...
	switch(aCommand)
	{
		...
		case E_TOGGLE_CUSTODY_ACCEPTENCE_SRR_POLICY:
			if(iBPA->GetSRRCustodyAcceptence())
			{
				iBPA->SetSRRCustodyAcceptence(EFalse);
				iAppView->PrintMsg(_L("BPA Custody Acceptance SRR Policy set to FALSE..."));
			}
			else
			{
				iBPA->SetSRRCustodyAcceptence(ETrue);
...
void CBPAUi::SendFileL()
{
	...
	if(fileData->Size() > 0)
	{
		if(!iBPA->Push(mergedData, ptrDest, 1, ETrue))
		{
								...
								TDesC8*& aHdrOut, TDesC8*& aDestOut)
{
	...
	TTime now;
	now.UniversalTime();
	_LIT(KDateTimeFormatString, "%E%D%X%N%Y %1 %2 %3 %-B%:0%J%:1%T%:2%S%:3%+B");
	...
	now.FormatL(bufTime, KDateTimeFormatString);
...
void CBPAUi::PrintBTAddr()
{
	...
	iAppView->PrintMsg(_L("Call to *#2820# to get BT Addr..."));
