...
FILE NAME	:	BTRFCommCLA.cpp

DESCRIPTION	:	This file implements BTRFCommCLA class, which specializes Convergence
				Layer Adapter's operation for BTRFComm.
...
CBTRFCommCLA* CBTRFCommCLA::NewL(const TUint32 aTimerPeriod, const TInt32 aKeepAliveInterval,
					   const TUint32 aPartialAckLen, const TUint8 aFlags, const TUint8 aMaxRetries,
					   const TGlobalConfigParams& aConfig)
{
	CBTRFCommCLA* self = CBTRFCommCLA::NewLC(aTimerPeriod, aKeepAliveInterval, aPartialAckLen, aFlags, aMaxRetries, aConfig);
...
CBTRFCommCLA* CBTRFCommCLA::NewLC(const TUint32 aTimerPeriod, const TInt32 aKeepAliveInterval,
						const TUint32 aPartialAckLen, const TUint8 aFlags, const TUint8 aMaxRetries,
						const TGlobalConfigParams& aConfig)
{
	CBTRFCommCLA* self = new (ELeave) CBTRFCommCLA(aKeepAliveInterval, aPartialAckLen, aFlags, aMaxRetries, aConfig);
...
TInt CBTRFCommCLA::StartListening(TUint32 aPort)
{
	...
	// open socket
	///retVal = iListeningSocket.Open(iSockServer, KBTAddrFamily, KSockStream, KRFCOMM);
	retVal = iListeningSocket.Open(iSockServer,_L("RFCOMM"));
	if(KErrNone != retVal)
		return retVal;
	...
	TBTSockAddr addr;
	addr.SetPort(aPort);
	retVal = iListeningSocket.Bind(addr);
...
void CBTRFCommCLA::EventHandler(TAny* aLPVOID)
{
...
void CBTRFCommCLA::ConstructL(const TUint32 aTimerPeriod)
{
...
void CBTRFCommCLA::RunL()
{
	TBuf<60> msg;
	msg.Copy(_L("error code in BT CLA RunL: "));
	...
	if(KErrNone == iStatus.Int())
	{
		...
		// This is to avoid tempsock ownership transfers to two places: ClnupStk & Conn. obj
		
		if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
		{
			User::InfoPrint(_L("BT CLA new incoming connection"));
		}
		CConnectionInfo *pConnInfo = new (ELeave) CBTRFCommConnectionInfo;
		...
		// is already safe...
		pConnInfo->iSockInfo = new (ELeave) TBTSockAddr;
		// Get remote IP and store here... this info would be available even if
		// socket is closed here after
		if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
		{
			User::InfoPrint(_L("BT CLA going to get Remote Addr"));
		}
		iTempSock->RemoteName(*(static_cast<TBTSockAddr*>(pConnInfo->iSockInfo)));
		
		// No need to set following ptr to cleanup-stack, as it's holder object
		// is already safe...
		pConnInfo->iConnectionPtr = CBTRFCommConnection::NewL(iTempSock, *iConfigParams);
		pConnInfo->iConnectionPtr->ConnectedActivate();	// start receiving
		iTempSock = NULL;
		
		if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
		{
			User::InfoPrint(_L("BT CLA new connection created"));
		}
...
TInt CBTRFCommCLA::RunError(TInt aError)
{
...
CBTRFCommCLA::CBTRFCommCLA(const TInt32 aKeepAliveInterval, const TUint32 aPartialAckLen,
...
void CBTRFCommCLA::ResetTempSockL()
{
...
void CBTRFCommCLA::DoCLATxOperations()
{
				...
				else	// deliver the bundle
				{
					...
					if(KErrNone == error)
					{
						...
						if(pConnInfo->iConnectionPtr->Push(conOp))
						{
							...
							(static_cast<CBTRFCommConnectionInfo*>(pConnInfo))->iLastTxMoment.UniversalTime();
						}
...
void CBTRFCommCLA::DoCLARxOperations()
{
	CBTRFCommConnectionInfo* pConnInfo;
	TInt lp_indx;
	CConnection::TConnectionOperation conOp;
	...
	for(lp_indx = 0; lp_indx < iConnectionQ->Count(); lp_indx++)
	{
		...
		pConnInfo = static_cast<CBTRFCommConnectionInfo*>(iConnectionQ->At(lp_indx));
			...
			switch(conOp.iOpCode)
			{
				case CConnection::E_COC_CONNECTED_OK:
					...
					else
					{
						...
						User::Panic(_L("Must be in INIT state, just after BT connection established"), 420);
					}
					break;
						...
						if(conOp.iTransactionID != 0)
						{
							...
							if(!(pConnInfo->iFlags & K_FLAG_REQ_ACK))
							{
								...
								else
								{
									...
									{
										User::InfoPrint(_L("BT::DoCLARxOperations :: invalid msg token from connection..."));
									}
								}
							}
					...
					else
					{
						...
						else
						{
							...
							if(NULL != tmp)
							{
								...
								TPtr8 ptrBuff = pConnInfo->iFragmentBuffer->Des();
								ptrBuff.Append(*(conOp.iData));
							}
							else
							{
...
void CBTRFCommCLA::DoCLAAuxillaryOperations()
{
	// Here we perform connection management operations:
	
	CBTRFCommConnectionInfo* pConnInfo = NULL;
	TInt lp_indx;
	TTime now;
	...
	for(lp_indx = 0; lp_indx < iConnectionQ->Count(); lp_indx++)
	{
		...
		else if(CConnectionInfo::E_CON_TERMINATED == pConnInfo->iConType)
		{
			if(pConnInfo->iConnectionPtr->IsRxQEmpty() && pConnInfo->iConnectionPtr->IsTxQEmpty())
			{
				...
				// now array indeces have changed, so
				lp_indx--;
				continue;
			}
		}
...
CConnectionInfo* CBTRFCommCLA::LocateConnection(TSockAddr* aAddr,
										   const CConnectionInfo::TConnectionType aConType)
{
	CConnectionInfo* pConnInfo = NULL;
	TBTSockAddr *tempAddrIn = static_cast<TBTSockAddr*>(aAddr);
	TBTSockAddr *tempAddr;
	...
	for(TInt lp_indx = 0; lp_indx < iConnectionQ->Count(); lp_indx++)
	{
		...
		// match based on MAC address only (as remote port can be arbitrary) and
		// connection type
		tempAddr = static_cast<TBTSockAddr*>(pConnInfo->iSockInfo);
		
		if( (tempAddr->BTAddr().Des() == tempAddrIn->BTAddr().Des()) &&
...
CConnectionInfo* CBTRFCommCLA::CreateConnectionL(TSockAddr* aAddr)
{
	...
	/*User::LeaveIfError(tmpSock->Open(iSockServer, KBTAddrFamily, KSockStream,
									 KRFCOMM) );*/
	User::LeaveIfError(tmpSock->Open(iSockServer, _L("RFCOMM")));
	...
	// Make a copy of remote Addr here (as we know the destination)...
	// this info would be available even if socket is closed here after
	pConnInfo->iSockInfo = static_cast<TSockAddr*>(new (ELeave) TBTSockAddr(*aAddr));
	pConnInfo->iConnectionPtr = CBTRFCommConnection::NewL(tmpSock, *iConfigParams);
...
void CBTRFCommCLA::ComposeCLADataUnitL(TBundleInfo& aBundleInfo, HBufC8*& aDataOut)
{
	...
	TInt totLen = static_cast<TInt>(lenStr->ByteStrLen()) + 
				  static_cast<TInt>(len) + K_BT_CLA_MSG_TYPE_LEN + K_BT_CLA_BUNDLE_ID_LEN;
...
void CBTRFCommCLA::ComposeACKMsgL(const TBundleInfo& aBundleInfo, const TUint32 aAckedLen, HBufC8*& aDataOut)
{
	CSDNV *lenStr = CSDNV::NewLC(aAckedLen);
	
	TInt totLen = static_cast<TInt>(lenStr->ByteStrLen()) + 
				  K_BT_CLA_MSG_TYPE_LEN + K_BT_CLA_BUNDLE_ID_LEN;
...
TBool CBTRFCommCLA::ComposeContactHeader(const CBTRFCommConnectionInfo *aConInfo, HBufC8*& aDataOut)
{
	...
	if(aConInfo->iConnectionInitiator)
	{
		...
		(const_cast<CBTRFCommConnectionInfo*>(aConInfo))->iFlags = tempFlags;
	}
	else
	{
	...
	HBufC8* tempHeader;
	tempHeader = HBufC8::New(K_BT_CLA_CONTACT_HEADER_SIZE);
	if(NULL == tempHeader)
		return EFalse;
	
	TPtr8 ptrHeader = tempHeader->Des();
	
	ptrHeader.Append(K_BT_CLA_MAGIC_STR);
...
void CBTRFCommCLA::HandleBundleDataMsgL(CBTRFCommConnectionInfo* pConnInfo)
{
	if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
	{
...
void CBTRFCommCLA::HandleBundleAckMsgL(CBTRFCommConnectionInfo* pConnInfo)
{
	if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
	{
...
void CBTRFCommCLA::HandleKeepAliveMsgL(CBTRFCommConnectionInfo* pConnInfo)
{
	if(iConfigParams->iLogLevel & K_LOG_BT_CLA)
	{
...
void CBTRFCommCLA::HandleShutDownMsgL(CBTRFCommConnectionInfo* pConnInfo)
{
	// To handle fragmentation and aggregation, we return normally
...
TBool CBTRFCommCLA::SendContactHeader(CBTRFCommConnectionInfo* pConnInfo)
{
...
TBool CBTRFCommCLA::HandleContactHeaderMsg(CBTRFCommConnectionInfo* pConnInfo, const TDesC8* aDataIn)
{
	...
	// match magic string
	if(0 != aDataIn->Left(K_BT_CLA_MAGIC_LEN).Compare(K_BT_CLA_MAGIC_STR))
	{
		return EFalse;
	}
...
TBool CBTRFCommCLA::AcknowledgeBundleTransmission(const TInt aIndex, const TBool aTransferStatus,
												  TUint32 aAckedLen)
{
...
void CBTRFCommCLA::AcknowledgeBundleReception(const TTransactionID aTransactionID, const TBool aTransferStatus)
{
...
TBool CBTRFCommCLA::ProcessIncomingMsg(CBTRFCommConnectionInfo* pConnInfo)
{
	TUint8 temp8;
